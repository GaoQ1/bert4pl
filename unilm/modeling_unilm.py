...
from transformers.modeling_bert import load_tf_weights_in_bert, BertPooler, BertIntermediate, BertOutput, BertPredictionHeadTransform, BertSelfOutput, BertLMPredictionHead, BertOnlyMLMHead, BertEmbeddings, BertOnlyNSPHead
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None):
        ...
        attention_scores = attention_scores + attention_mask # attention_mask会将超出max_length的部分设为-1000意思就是忽略这部分
...
class LabelSmoothingLoss(_Loss):
    ...
    def forward(self, output, target):
        ...
        model_prob.scatter_(1, target.unsqueeze(1), self.confidence) # scatter_(input, dim, index, src)将src中数据根据index中的索引按照dim的方向填进input中
        model_prob.masked_fill_((target == self.ignore_index).unsqueeze(1), 0) # 是将mask中为1的元素所在的索引，在a中相同的的索引处替换为value
